// Written by Gregory Nutt using the System V ABI Edition 4.1
...
#define SHT_ARM_PREEMPTMAP                                                     \
    0x70000002 // BPABI DLL dynamic linking pre-emption map
...
// 0xFFFFFFFC). T is 1 if the target symbol S has type STT_FUNC and the symbol
// addresses a Thumb instruction; it is 0 otherwise. B(S) is the addressing
// origin of the output segment defining the symbol S. GOT_ORG is the addressing
// origin of the Global Offset Table GOT(S) is the address of the GOT entry for
// the symbol S.
