...
void ULPIConfigSet(uint32_t ui32Base, uint32_t ui32Config) {
    ...
    ui8Val &=
        ~(ULPI_ICTL_AUTORESUME | ULPI_ICTL_INDINV | ULPI_ICTL_INDPASSTHRU);
...
void ULPIPowerTransceiver(uint32_t ui32Base, bool bEnable) {
    if (bEnable) {
        USBULPIRegWrite(ui32Base, ULPI_FCTL_CLEAR, ULPI_FCTL_OPMODE_SUSPEND);
    } else {
        USBULPIRegWrite(ui32Base, ULPI_FCTL_SET, ULPI_FCTL_OPMODE_SUSPEND);
    }
}
