...
int ESP8266_Connect(int verbose){
  ...
  if(verbose)  // debug output: MAC address oF ESP8266
    if (ESP8266_GetMACAddress()==FAILURE) return FAILURE; 

  if(verbose)  // debug output: see APs in area
    if(ESP8266_ListAccessPoints()==FAILURE) return FAILURE; 
  ...
  if(verbose)  // debug output: our IP address
    if(ESP8266_GetIPAddress()==FAILURE) return FAILURE;
...
// output: 1 if success, 0 if fail 
int ESP8266_MakeTCPConnection(char *IPaddress, uint16_t port, uint16_t keepalive){ int try=MAXTRY;
  char TXBuffer[32];
  while(try){
