...
static int lfs_bd_read(lfs_t* lfs, const lfs_cache_t* pcache,
                       lfs_cache_t* rcache, lfs_size_t hint, lfs_block_t block,
    ...
    while (size > 0) {
        ...
        rcache->size =
            lfs_min(lfs_min(lfs_alignup(off + hint, lfs->cfg->read_size),
                            lfs->cfg->block_size) -
                        rcache->off,
                    lfs->cfg->cache_size);
...
static int lfs_bd_cmp(lfs_t* lfs, const lfs_cache_t* pcache,
                      lfs_cache_t* rcache, lfs_size_t hint, lfs_block_t block,
    ...
    for (lfs_off_t i = 0; i < size; i++) {
        uint8_t dat;
        int err =
            lfs_bd_read(lfs, pcache, rcache, hint - i, block, off + i, &dat, 1);
        if (err) {
            return err;
        }
...
static int lfs_bd_flush(lfs_t* lfs, lfs_cache_t* pcache, lfs_cache_t* rcache,
                        bool validate) {
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        ...
        if (validate) {
            ...
            int res = lfs_bd_cmp(lfs, NULL, rcache, diff, pcache->block,
...
static int lfs_bd_sync(lfs_t* lfs, lfs_cache_t* pcache, lfs_cache_t* rcache,
                       bool validate) {
    lfs_cache_drop(lfs, rcache);
...
static int lfs_bd_prog(lfs_t* lfs, lfs_cache_t* pcache, lfs_cache_t* rcache,
                       bool validate, lfs_block_t block, lfs_off_t off,
    ...
    while (size > 0) {
        if (block == pcache->block && off >= pcache->off &&
            ...
            lfs_size_t diff =
                lfs_min(size, lfs->cfg->cache_size - (off - pcache->off));
...
static inline int lfs_pair_cmp(const lfs_block_t paira[2],
                               const lfs_block_t pairb[2]) {
    return !(paira[0] == pairb[0] || paira[1] == pairb[1] ||
             paira[0] == pairb[1] || paira[1] == pairb[0]);
}

static inline bool lfs_pair_sync(const lfs_block_t paira[2],
                                 const lfs_block_t pairb[2]) {
    return (paira[0] == pairb[0] && paira[1] == pairb[1]) ||
           (paira[0] == pairb[1] && paira[1] == pairb[0]);
}
...
static int lfs_dir_compact(lfs_t* lfs, lfs_mdir_t* dir,
                           const struct lfs_mattr* attrs, int attrcount,
...
static lfs_stag_t lfs_fs_parent(lfs_t* lfs, const lfs_block_t dir[2],
                                lfs_mdir_t* parent);
static int lfs_fs_relocate(lfs_t* lfs, const lfs_block_t oldpair[2],
                           lfs_block_t newpair[2]);
int lfs_fs_traverseraw(lfs_t* lfs, int (*cb)(void* data, lfs_block_t block),
...
static int lfs_alloc_lookahead(void* p, lfs_block_t block) {
    lfs_t* lfs = (lfs_t*)p;
    lfs_block_t off = ((block - lfs->free.off) + lfs->cfg->block_count) %
                      lfs->cfg->block_count;
...
static int lfs_alloc(lfs_t* lfs, lfs_block_t* block) {
    while (true) {
        ...
        lfs->free.off =
            (lfs->free.off + lfs->free.size) % lfs->cfg->block_count;
...
static lfs_stag_t lfs_dir_getslice(lfs_t* lfs, const lfs_mdir_t* dir,
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        lfs_tag_t tag = ntag;
        int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(ntag),
        ...
        if ((gmask & tag) == (gmask & (gtag - gdiff))) {
            ...
            err = lfs_bd_read(lfs, NULL, &lfs->rcache, diff, dir->pair[0],
                              off + sizeof(tag) + goff, gbuffer, diff);
            if (err) {
                return err;
            }
...
static int lfs_dir_getread(lfs_t* lfs, const lfs_mdir_t* dir,
                           ...
                           lfs_size_t hint, lfs_tag_t gmask, lfs_tag_t gtag,
                           lfs_off_t off, void* buffer, lfs_size_t size) {
    uint8_t* data = buffer;
    ...
    while (size > 0) {
        ...
        rcache->size = lfs_min(lfs_alignup(off + hint, lfs->cfg->read_size),
                               lfs->cfg->cache_size);
        int err = lfs_dir_getslice(lfs, dir, gmask, gtag, rcache->off,
                                   rcache->buffer, rcache->size);
        if (err < 0) {
            return err;
        }
...
static int lfs_dir_traverse_filter(void* p, lfs_tag_t tag, const void* buffer) {
    ...
    uint32_t mask = (tag & LFS_MKTAG(0x100, 0, 0)) ? LFS_MKTAG(0x7ff, 0x3ff, 0)
...
static int lfs_dir_traverse(
    lfs_t* lfs, const lfs_mdir_t* dir, lfs_off_t off, lfs_tag_t ptag,
    ...
    while (true) {
        ...
        if (lfs_tag_id(tmask) != 0) {
            ...
            int filter =
                lfs_dir_traverse(lfs, dir, off, ptag, attrs, attrcount, 0, 0, 0,
                                 0, 0, lfs_dir_traverse_filter, &tag);
            if (filter < 0) {
                return filter;
            }
        ...
        if (lfs_tag_type3(tag) == LFS_FROM_NOOP) {
            ...
            int err = lfs_dir_traverse(
                lfs, buffer, 0, 0xffffffff, NULL, 0, LFS_MKTAG(0x600, 0x3ff, 0),
            ...
            for (unsigned i = 0; i < lfs_tag_size(tag); i++) {
                ...
                int err = cb(data,
                             LFS_MKTAG(LFS_TYPE_USERATTR + a[i].type,
...
static lfs_stag_t
lfs_dir_fetchmatch(lfs_t* lfs, lfs_mdir_t* dir, const lfs_block_t pair[2],
    ...
    int r = 0;
    for (int i = 0; i < 2; i++) {
        int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(revs[i]), pair[i],
                              0, &revs[i], sizeof(revs[i]));
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            lfs_tag_t tag;
            off += lfs_tag_dsize(ptag);
            int err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size,
                                  dir->pair[0], off, &tag, sizeof(tag));
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    // can't continue?
            ...
            if (lfs_tag_type1(tag) == LFS_TYPE_CRC) {
                // check the crc attr
                uint32_t dcrc;
                err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size,
                                  dir->pair[0], off + sizeof(tag), &dcrc,
                                  sizeof(dcrc));
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        dir->erased = false;
            ...
            if (lfs_tag_type1(tag) == LFS_TYPE_NAME) {
                ...
                err =
                    lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size,
            ...
            if ((fmask & tag) == (fmask & ftag)) {
                int res =
                    cb(data, tag,
                       &(struct lfs_diskoff){dir->pair[0], off + sizeof(tag)});
                if (res < 0) {
...
static int lfs_dir_fetch(lfs_t* lfs, lfs_mdir_t* dir,
                         const lfs_block_t pair[2]) {
    // note, mask=-1, tag=-1 can never match a tag since this
    // pattern has the invalid bit set
    return (int)lfs_dir_fetchmatch(lfs, dir, pair, (lfs_tag_t)-1, (lfs_tag_t)-1,
                                   NULL, NULL, NULL);
}
...
static int lfs_dir_getgstate(lfs_t* lfs, const lfs_mdir_t* dir,
    ...
    lfs_gstate_t temp;
    lfs_stag_t res =
        lfs_dir_get(lfs, dir, LFS_MKTAG(0x7ff, 0, 0),
...
static int lfs_dir_getinfo(lfs_t* lfs, lfs_mdir_t* dir, uint16_t id,
    ...
    lfs_stag_t tag = lfs_dir_get(
        lfs, dir, LFS_MKTAG(0x780, 0x3ff, 0),
...
static int lfs_dir_find_match(void* data, lfs_tag_t tag, const void* buffer) {
    ...
    int res = lfs_bd_cmp(lfs, NULL, &lfs->rcache, diff, disk->block, disk->off,
                         name->name, diff);
    if (res != LFS_CMP_EQ) {
        return res;
    }
...
static lfs_stag_t lfs_dir_find(lfs_t* lfs, lfs_mdir_t* dir, const char** path,
                               uint16_t* id) {
        ...
        *path = name;
        ...
        if (lfs_tag_id(tag) != 0x3ff) {
            lfs_stag_t res = lfs_dir_get(
                lfs, dir, LFS_MKTAG(0x700, 0x3ff, 0),
        ...
        while (true) {
            tag = lfs_dir_fetchmatch(
                lfs, dir, dir->tail, LFS_MKTAG(0x780, 0, 0),
...
static int lfs_dir_commitprog(lfs_t* lfs, struct lfs_commit* commit,
                              const void* buffer, lfs_size_t size) {
    int err = lfs_bd_prog(lfs, &lfs->pcache, &lfs->rcache, false, commit->block,
                          commit->off, (const uint8_t*)buffer, size);
    if (err) {
        return err;
    }
...
static int lfs_dir_commitcrc(lfs_t* lfs, struct lfs_commit* commit) {
    ...
    const lfs_off_t end =
        lfs_alignup(off1 + 2 * sizeof(uint32_t), lfs->cfg->prog_size);
    ...
    while (commit->off < end) {
        ...
        err = lfs_bd_prog(lfs, &lfs->pcache, &lfs->rcache, false, commit->block,
                          commit->off, &footer, sizeof(footer));
        if (err) {
            return err;
        }
    ...
    while (off < end) {
        ...
        for (lfs_off_t i = off; i < noff + sizeof(uint32_t); i++) {
            ...
            uint8_t dat;
            err = lfs_bd_read(lfs, NULL, &lfs->rcache,
                              noff + sizeof(uint32_t) - i, commit->block, i,
                              &dat, 1);
            if (err) {
                return err;
            }
...
static int lfs_dir_alloc(lfs_t* lfs, lfs_mdir_t* dir) {
    ...
    int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(dir->rev),
...
static int lfs_dir_split(lfs_t* lfs, lfs_mdir_t* dir,
                         const struct lfs_mattr* attrs, int attrcount,
...
static int lfs_dir_compact(lfs_t* lfs, lfs_mdir_t* dir,
    ...
    while (end - begin > 1) {
        // find size
        lfs_size_t size = 0;
        int err = lfs_dir_traverse(lfs, source, 0, 0xffffffff, attrs, attrcount,
                                   LFS_MKTAG(0x400, 0x3ff, 0),
                                   LFS_MKTAG(LFS_TYPE_NAME, 0, 0), begin, end,
                                   -begin, lfs_dir_commit_size, &size);
        if (err) {
            return err;
        }
        ...
        err = lfs_dir_split(lfs, dir, attrs, attrcount, source, begin + split,
                            end);
        if (err) {
    ...
    if (lfs->cfg->block_cycles > 0 &&
        ...
        if (lfs_pair_cmp(dir->pair, (const lfs_block_t[2]){0, 1}) == 0) {
            ...
            if ((lfs_size_t)res < lfs->cfg->block_count / 2) {
                ...
                int err = lfs_dir_split(lfs, dir, attrs, attrcount, source,
                                        begin, end);
    ...
    while (true) {
        {
            ...
            err = lfs_dir_traverse(
                lfs, source, 0, 0xffffffff, attrs, attrcount,
            ...
            if (!lfs_pair_isnull(dir->tail)) {
                ...
                err = lfs_dir_commitattr(
                    lfs, &commit,
                    LFS_MKTAG(LFS_TYPE_TAIL + dir->split, 0x3ff, 8), dir->tail);
                lfs_pair_fromle32(dir->tail);
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        goto relocate;
            ...
            if (!lfs_gstate_iszero(&delta)) {
                ...
                err = lfs_dir_commitattr(
                    lfs, &commit,
                    LFS_MKTAG(LFS_TYPE_MOVESTATE, 0x3ff, sizeof(delta)),
                    &delta);
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        goto relocate;
...
static int lfs_dir_commit(lfs_t* lfs, lfs_mdir_t* dir,
    ...
    if (dir->erased || dir->count >= 0xff) {
        ...
        lfs_pair_tole32(dir->tail);
        int err =
            lfs_dir_traverse(lfs, dir, dir->off, dir->etag, attrs, attrcount, 0,
                             0, 0, 0, 0, lfs_dir_commit_commit,
                             &(struct lfs_dir_commit_commit){lfs, &commit});
        lfs_pair_fromle32(dir->tail);
        if (err) {
    ...
    compact:
        ...
        int err =
            lfs_dir_compact(lfs, dir, attrs, attrcount, dir, 0, dir->count);
        if (err) {
            *dir = olddir;
            return err;
        }
...
int lfs_mkdir(lfs_t* lfs, const char* path) {
    ...
    err = lfs_dir_commit(
        lfs, &dir,
        LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), pred.tail}));
    ...
    if (cwd.m.split) {
        ...
        err = lfs_dir_commit(
            lfs, &pred,
            LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), dir.pair}));
    ...
    err = lfs_dir_commit(
        ...
        LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_CREATE, id, 0), NULL},
                    ...
                    {LFS_MKTAG_IF(!cwd.m.split, LFS_TYPE_SOFTTAIL, 0x3ff, 8),
                     dir.pair}));
    lfs_pair_fromle32(dir.pair);
    if (err) {
        LFS_TRACE("lfs_mkdir -> %d", err);
...
int lfs_dir_open(lfs_t* lfs, lfs_dir_t* dir, const char* path) {
    ...
    if (lfs_tag_id(tag) == 0x3ff) {
        ...
        lfs_stag_t res =
            lfs_dir_get(lfs, &dir->m, LFS_MKTAG(0x700, 0x3ff, 0),
...
static int lfs_ctz_find(lfs_t* lfs, const lfs_cache_t* pcache,
                        lfs_cache_t* rcache, lfs_block_t head, lfs_size_t size,
    ...
    while (current > target) {
        ...
        int err = lfs_bd_read(lfs, pcache, rcache, sizeof(head), head, 4 * skip,
                              &head, sizeof(head));
...
static int lfs_ctz_extend(lfs_t* lfs, lfs_cache_t* pcache, lfs_cache_t* rcache,
...

        {
            ...
            if (noff != lfs->cfg->block_size) {
                for (lfs_off_t i = 0; i < noff; i++) {
                    uint8_t data;
                    err = lfs_bd_read(lfs, NULL, rcache, noff - i, head, i,
                                      &data, 1);
                    if (err) {
                        return err;
                    }

                    err = lfs_bd_prog(lfs, pcache, rcache, true, nblock, i,
                                      &data, 1);
                    if (err) {
                        if (err == LFS_ERR_CORRUPT) {
                            goto relocate;
            ...
            for (lfs_off_t i = 0; i < skips; i++) {
                ...
                err = lfs_bd_prog(lfs, pcache, rcache, true, nblock, 4 * i,
                                  &nhead, 4);
                ...
                if (i != skips - 1) {
                    err = lfs_bd_read(lfs, NULL, rcache, sizeof(nhead), nhead,
                                      4 * i, &nhead, sizeof(nhead));
                    nhead = lfs_fromle32(nhead);
                    if (err) {
                        return err;
                    }
...
static int lfs_ctz_traverse(lfs_t* lfs, const lfs_cache_t* pcache,
                            lfs_cache_t* rcache, lfs_block_t head,
    ...
    while (true) {
        ...
        lfs_block_t heads[2];
        int count = 2 - (index & 1);
        err = lfs_bd_read(lfs, pcache, rcache, count * sizeof(head), head, 0,
                          &heads, count * sizeof(head));
...
int lfs_file_opencfg(lfs_t* lfs, lfs_file_t* file, const char* path, int flags,
    ...
    LFS_TRACE("lfs_file_opencfg(%p, %p, \"%s\", %x, %p {"
              ...
              (void*)lfs, (void*)file, path, flags, (void*)cfg, cfg->buffer,
              (void*)cfg->attrs, cfg->attr_count);
    ...
    for (unsigned i = 0; i < file->cfg->attr_count; i++) {
        if ((file->flags & 3) != LFS_O_WRONLY) {
            lfs_stag_t res = lfs_dir_get(
                lfs, &file->m, LFS_MKTAG(0x7ff, 0x3ff, 0),
    ...
    while (true) {
        ...
        for (lfs_off_t i = 0; i < file->off; i++) {
            ...
            if (file->flags & LFS_F_INLINE) {
                ...
                err = lfs_bd_read(lfs, &file->cache, &lfs->rcache,
                                  file->off - i, file->block, i, &data, 1);
                if (err) {
                    return err;
                }
            }

            err = lfs_bd_prog(lfs, &lfs->pcache, &lfs->rcache, true, nblock, i,
                              &data, 1);
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    goto relocate;
    ...
    if ((file->flags & LFS_F_DIRTY) && !lfs_pair_isnull(file->m.pair)) {
        ...
        err = lfs_dir_commit(
            ...
            LFS_MKATTRS(
                ...
                {LFS_MKTAG(LFS_FROM_USERATTRS, file->id, file->cfg->attr_count),
                 file->cfg->attrs}));
        if (err) {
            file->flags |= LFS_F_ERRED;
            LFS_TRACE("lfs_file_sync -> %d", err);
...
lfs_ssize_t lfs_file_read(lfs_t* lfs, lfs_file_t* file, void* buffer,
                          lfs_size_t size) {
    LFS_TRACE("lfs_file_read(%p, %p, %p, %" PRIu32 ")", (void*)lfs, (void*)file,
              buffer, size);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    ...
    while (nsize > 0) {
        ...
        if (!(file->flags & LFS_F_READING) ||
            ...
            if (!(file->flags & LFS_F_INLINE)) {
                int err = lfs_ctz_find(lfs, NULL, &file->cache, file->ctz.head,
                                       file->ctz.size, file->pos, &file->block,
                                       &file->off);
                if (err) {
                    LFS_TRACE("lfs_file_read -> %d", err);
                    return err;
        ...
        if (file->flags & LFS_F_INLINE) {
            int err = lfs_dir_getread(
                lfs, &file->m, NULL, &file->cache, lfs->cfg->block_size,
                LFS_MKTAG(0xfff, 0x1ff, 0),
                LFS_MKTAG(LFS_TYPE_INLINESTRUCT, file->id, 0), file->off, data,
                diff);
            if (err) {
                LFS_TRACE("lfs_file_read -> %d", err);
                return err;
            ...
            int err = lfs_bd_read(lfs, NULL, &file->cache, lfs->cfg->block_size,
...
lfs_ssize_t lfs_file_write(lfs_t* lfs, lfs_file_t* file, const void* buffer,
                           lfs_size_t size) {
    LFS_TRACE("lfs_file_write(%p, %p, %p, %" PRIu32 ")", (void*)lfs,
              (void*)file, buffer, size);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    ...
    if ((file->flags & LFS_F_INLINE) &&
        lfs_max(file->pos + nsize, file->ctz.size) >
            lfs_min(0x3fe,
                    lfs_min(lfs->cfg->cache_size, lfs->cfg->block_size / 8))) {
            ...
            if (!(file->flags & LFS_F_INLINE)) {
                if (!(file->flags & LFS_F_WRITING) && file->pos > 0) {
                    ...
                    int err = lfs_ctz_find(
                        lfs, NULL, &file->cache, file->ctz.head, file->ctz.size,
                ...
                lfs_alloc_ack(lfs);
                int err =
                    lfs_ctz_extend(lfs, &file->cache, &lfs->rcache, file->block,
...
lfs_soff_t lfs_file_seek(lfs_t* lfs, lfs_file_t* file, lfs_soff_t off,
                         int whence) {
    LFS_TRACE("lfs_file_seek(%p, %p, %" PRId32 ", %d)", (void*)lfs, (void*)file,
              off, whence);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
...
int lfs_file_truncate(lfs_t* lfs, lfs_file_t* file, lfs_off_t size) {
    LFS_TRACE("lfs_file_truncate(%p, %p, %" PRIu32 ")", (void*)lfs, (void*)file,
              size);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
    LFS_ASSERT((file->flags & 3) != LFS_O_RDONLY);
    ...
    if (size < oldsize) {
        ...
        err = lfs_ctz_find(lfs, NULL, &file->cache, file->ctz.head,
                           file->ctz.size, size, &file->block, &file->off);
        if (err) {
            LFS_TRACE("lfs_file_truncate -> %d", err);
            return err;
        }
...
int lfs_remove(lfs_t* lfs, const char* path) {
    ...
    if (lfs_tag_type3(tag) == LFS_TYPE_DIR) {
        ...
        lfs_block_t pair[2];
        lfs_stag_t res =
            lfs_dir_get(lfs, &cwd, LFS_MKTAG(0x700, 0x3ff, 0),
                        LFS_MKTAG(LFS_TYPE_STRUCT, lfs_tag_id(tag), 8), pair);
        if (res < 0) {
    ...
    err = lfs_dir_commit(
        lfs, &cwd,
        LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_DELETE, lfs_tag_id(tag), 0), NULL}));
    if (err) {
        lfs->mlist = dir.next;
        LFS_TRACE("lfs_remove -> %d", err);
        return err;
...
int lfs_rename(lfs_t* lfs, const char* oldpath, const char* newpath) {
    ...
    if (prevtag == LFS_ERR_NOENT) {
        ...
        lfs_stag_t res =
            lfs_dir_get(lfs, &newcwd, LFS_MKTAG(0x700, 0x3ff, 0),
    ...
    err = lfs_dir_commit(
        lfs, &newcwd,
        LFS_MKATTRS(
            {LFS_MKTAG_IF(prevtag != LFS_ERR_NOENT, LFS_TYPE_DELETE, newid, 0),
             NULL},
...
lfs_ssize_t lfs_getattr(lfs_t* lfs, const char* path, uint8_t type,
    ...
    tag = lfs_dir_get(
        lfs, &cwd, LFS_MKTAG(0x7ff, 0x3ff, 0),
...
static int lfs_commitattr(lfs_t* lfs, const char* path, uint8_t type,
                          const void* buffer, lfs_size_t size) {
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
...
int lfs_setattr(lfs_t* lfs, const char* path, uint8_t type, const void* buffer,
                lfs_size_t size) {
...
int lfs_format(lfs_t* lfs, const struct lfs_config* cfg) {
    LFS_TRACE("lfs_format(%p, %p {.context=%p, "
              ...
              ".attr_max=%" PRIu32 "})",
              (void*)lfs, (void*)cfg, cfg->context, (void*)(uintptr_t)cfg->read,
              (void*)(uintptr_t)cfg->prog, (void*)(uintptr_t)cfg->erase,
              (void*)(uintptr_t)cfg->sync, cfg->read_size, cfg->prog_size,
              cfg->block_size, cfg->block_count, cfg->block_cycles,
              cfg->cache_size, cfg->lookahead_size, cfg->read_buffer,
    ...
    int err = 0;
    {
        ...
        lfs->free.off = 0;
        lfs->free.size =
            lfs_min(8 * lfs->cfg->lookahead_size, lfs->cfg->block_count);
        lfs->free.i = 0;
        lfs_alloc_ack(lfs);
...
int lfs_mount(lfs_t* lfs, const struct lfs_config* cfg) {
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        lfs_stag_t tag = lfs_dir_fetchmatch(
            lfs, &dir, dir.tail, LFS_MKTAG(0x7ff, 0x3ff, 0),
            LFS_MKTAG(LFS_TYPE_SUPERBLOCK, 0, 8), NULL, lfs_dir_find_match,
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            lfs_superblock_t superblock;
            tag = lfs_dir_get(
                lfs, &dir, LFS_MKTAG(0x7ff, 0x3ff, 0),
                LFS_MKTAG(LFS_TYPE_INLINESTRUCT, 0, sizeof(superblock)),
                &superblock);
            if (tag < 0) {
                err = tag;
...
int lfs_fs_traverseraw(lfs_t* lfs, int (*cb)(void* data, lfs_block_t block),
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        for (uint16_t id = 0; id < dir.count; id++) {
            struct lfs_ctz ctz;
            lfs_stag_t tag =
                lfs_dir_get(lfs, &dir, LFS_MKTAG(0x700, 0x3ff, 0),
                            LFS_MKTAG(LFS_TYPE_STRUCT, id, sizeof(ctz)), &ctz);
            if (tag < 0) {
                if (tag == LFS_ERR_NOENT) {
                    continue;
            ...
            if (lfs_tag_type3(tag) == LFS_TYPE_CTZSTRUCT) {
                err = lfs_ctz_traverse(lfs, NULL, &lfs->rcache, ctz.head,
                                       ctz.size, cb, data);
                if (err) {
                    return err;
                }
    ...
    for (lfs_file_t* f = (lfs_file_t*)lfs->mlist; f; f = f->next) {
        ...
        if ((f->flags & LFS_F_WRITING) && !(f->flags & LFS_F_INLINE)) {
            int err = lfs_ctz_traverse(lfs, &f->cache, &lfs->rcache, f->block,
                                       f->pos, cb, data);
            if (err) {
                return err;
            }
...
int lfs_fs_traverse(lfs_t* lfs, int (*cb)(void* data, lfs_block_t block),
                    void* data) {
    LFS_TRACE("lfs_fs_traverse(%p, %p, %p)", (void*)lfs, (void*)(uintptr_t)cb,
              data);
...
static int lfs_fs_pred(lfs_t* lfs, const lfs_block_t pair[2],
                       lfs_mdir_t* pdir) {
...
static int lfs_fs_parent_match(void* data, lfs_tag_t tag, const void* buffer) {
    ...
    lfs_block_t child[2];
    int err = lfs_bd_read(lfs, &lfs->pcache, &lfs->rcache, lfs->cfg->block_size,
...
static lfs_stag_t lfs_fs_parent(lfs_t* lfs, const lfs_block_t pair[2],
    ...
    while (!lfs_pair_isnull(parent->tail)) {
        ...
        lfs_stag_t tag = lfs_dir_fetchmatch(
            lfs, parent, parent->tail, LFS_MKTAG(0x7ff, 0, 0x3ff),
            LFS_MKTAG(LFS_TYPE_DIRSTRUCT, 0, 8), NULL, lfs_fs_parent_match,
...
static int lfs_fs_relocate(lfs_t* lfs, const lfs_block_t oldpair[2],
    ...
    if (tag != LFS_ERR_NOENT) {
        ...
        int err = lfs_dir_commit(
            lfs, &parent,
            LFS_MKATTRS(
                {LFS_MKTAG_IF(moveid != 0x3ff, LFS_TYPE_DELETE, moveid, 0),
                 NULL},
                {tag, newpair}));
...
static void lfs_fs_prepmove(lfs_t* lfs, uint16_t id,
                            const lfs_block_t pair[2]) {
...
static int lfs_fs_demove(lfs_t* lfs) {
    ...
    LFS_DEBUG("Fixing move {0x%" PRIx32 ", 0x%" PRIx32 "} 0x%" PRIx16,
              lfs->gdisk.pair[0], lfs->gdisk.pair[1],
    ...
    err = lfs_dir_commit(
        lfs, &movedir,
        LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_DELETE, moveid, 0), NULL}));
    if (err) {
        return err;
    }
...
static int lfs1_bd_read(lfs_t* lfs, lfs_block_t block, lfs_off_t off,
                        void* buffer, lfs_size_t size) {
    // if we ever do more than writes to alternating pairs,
    // this may need to consider pcache
    return lfs_bd_read(lfs, &lfs->pcache, &lfs->rcache, size, block, off,
                       buffer, size);
}

static int lfs1_bd_crc(lfs_t* lfs, lfs_block_t block, lfs_off_t off,
...
static int lfs1_dir_fetch(lfs_t* lfs, lfs1_dir_t* dir,
                          const lfs_block_t pair[2]) {
...
static int lfs1_dir_next(lfs_t* lfs, lfs1_dir_t* dir, lfs1_entry_t* entry) {
    ...
    int err =
        lfs1_bd_read(lfs, dir->pair[0], dir->off, &entry->d, sizeof(entry->d));
...
int lfs1_traverse(lfs_t* lfs, int (*cb)(void*, lfs_block_t), void* data) {
    ...
    while (true) {
        ...
        while (dir.off + sizeof(entry.d) <= (0x7fffffff & dir.d.size) - 4) {
            err = lfs1_bd_read(lfs, dir.pair[0], dir.off, &entry.d,
                               sizeof(entry.d));
            lfs1_entry_fromle32(&entry.d);
            if (err) {
                return err;
...
static int lfs1_mount(lfs_t* lfs, struct lfs1* lfs1,
    ...
    int err = 0;
    {
        ...
        if (!err) {
            err = lfs1_bd_read(lfs, dir.pair[0], sizeof(dir.d), &superblock.d,
                               sizeof(superblock.d));
        ...
        if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
            LFS_ERROR("Invalid superblock at {0x%" PRIx32 ", 0x%" PRIx32 "}", 0,
                      1);
            err = LFS_ERR_CORRUPT;
            goto cleanup;
        }
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail)) {
            ...
            while (true) {
                ...
                err = lfs_dir_commit(
                    lfs, &dir2,
                    LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_CREATE, id, 0)},
                                {LFS_MKTAG_IF_ELSE(isdir, LFS_TYPE_DIR, id,
                                                   entry1.d.nlen, LFS_TYPE_REG,
                                                   id, entry1.d.nlen),
                                 name},
                                {LFS_MKTAG_IF_ELSE(isdir, LFS_TYPE_DIRSTRUCT,
                                                   id, sizeof(entry1.d.u),
                                                   LFS_TYPE_CTZSTRUCT, id,
                                                   sizeof(entry1.d.u)),
                                 &entry1.d.u}));
            ...
            for (lfs_off_t i = 0; i < dir2.off; i++) {
                uint8_t dat;
                err = lfs_bd_read(lfs, NULL, &lfs->rcache, dir2.off,
                                  dir2.pair[0], i, &dat, 1);
                if (err) {
                    goto cleanup;
                }

                err = lfs_bd_prog(lfs, &lfs->pcache, &lfs->rcache, true,
                                  dir1.head[1], i, &dat, 1);
                if (err) {
                    goto cleanup;
                }
