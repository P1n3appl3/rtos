...
static void USBDGetConfiguration(void* pvInstance, tUSBRequest* psUSBRequest);
static void USBDSetConfiguration(void* pvInstance, tUSBRequest* psUSBRequest);
...
//
//*****************************************************************************
static const tStdRequest g_psUSBDStdRequests[] = {USBDGetStatus,
                                                  USBDClearFeature,
                                                  0,
                                                  USBDSetFeature,
                                                  0,
...
void USBDCDInit(uint32_t ui32Index, tDeviceInfo* psDevice, void* pvDCDCBData) {
    ...
    psHdr =
        psDevice
            ->ppsConfigDescriptors[g_psDCDInst[0].ui32DefaultConfiguration - 1];
    ...
    if (g_iUSBMode != eUSBModeOTG) {
        ...
        //
        MAP_USBIntEnableControl(USB0_BASE,
                                USB_INTCTRL_RESET | USB_INTCTRL_DISCONNECT |
...
//
//*****************************************************************************
void USBDCDRequestDataEP0(uint32_t ui32Index, uint8_t* pui8Data,
                          uint32_t ui32Size) {
    ASSERT(ui32Index == 0);
...
//
//*****************************************************************************
void USBDCDSendDataEP0(uint32_t ui32Index, uint8_t* pui8Data,
                       uint32_t ui32Size) {
    ASSERT(ui32Index == 0);
...
//
//*****************************************************************************
bool USBDCDFeatureSet(uint32_t ui32Index, uint32_t ui32Feature,
                      void* pvFeature) {
    bool bRetCode;
...
//
//*****************************************************************************
bool USBDCDFeatureGet(uint32_t ui32Index, uint32_t ui32Feature,
                      void* pvFeature) {
    bool bRetCode = true;
...
static void USBDReadAndDispatchRequest(uint32_t ui32Index) {
    ...
    //
    MAP_USBEndpointDataGet(USB0_BASE, USB_EP_0, g_pui8DataBufferIn, &ui32Size);
    ...
    if ((psRequest->bmRequestType & USB_RTYPE_TYPE_M) != USB_RTYPE_STANDARD) {
        ...
        if (g_ppsDevInfo[0]->psCallbacks->pfnRequestHandler) {
            g_ppsDevInfo[0]->psCallbacks->pfnRequestHandler(
                g_psDCDInst[0].pvCBData, psRequest);
        } else {
            //
...
void USBDeviceEnumHandler(tDCDInstance* pDevInstance) {
    ...
    case eUSBStateRx: {
        ...
        // Get the data from the USB controller end point 0.
        //
        MAP_USBEndpointDataGet(USB0_BASE, USB_EP_0, pDevInstance->pui8EP0Data,
                               &ui32DataSize);
        ...
        if (pDevInstance->ui32EP0DataRemain < EP0_MAX_PACKET_SIZE) {
            ...
            if ((g_ppsDevInfo[0]->psCallbacks->pfnDataReceived) &&
                ...
                g_ppsDevInfo[0]->psCallbacks->pfnDataReceived(
                    g_psDCDInst[0].pvCBData, pDevInstance->ui32OUTDataSize);
...
static void USBDGetStatus(void* pvInstance, tUSBRequest* psUSBRequest) {
    ...
    case USB_RTYPE_ENDPOINT: {
        ...
        if ((ui16Index == 0) || (ui16Index >= NUM_USB_EP)) {
            ...
            //
            ui32Dir =
                ((psUSBRequest->wIndex & USB_REQ_EP_DIR_M) == USB_REQ_EP_DIR_IN)
                    ? HALT_EP_IN
                    : HALT_EP_OUT;
            ...
            if (USB_FEATURE_EP_HALT == psUSBRequest->wValue) {
                ...
                if (ui32Dir == HALT_EP_IN) {
                    MAP_USBDevEndpointStallClear(
                        USB0_BASE, IndexToUSBEP(ui16Index), USB_EP_DEV_IN);
                } else {
                    MAP_USBDevEndpointStallClear(
                        USB0_BASE, IndexToUSBEP(ui16Index), USB_EP_DEV_OUT);
                }
            } else {
                //
...
static void USBDGetDescriptor(void* pvInstance, tUSBRequest* psUSBRequest) {
    ...
    case USB_DTYPE_DEVICE: {
        ...
        // Return the externally provided device descriptor.
        //
        psUSBControl->pui8EP0Data = (uint8_t*)psDevice->pui8DeviceDescriptor;

        //
        // The size of the device descriptor is in the first byte.
        //
        psUSBControl->ui32EP0DataRemain = psDevice->pui8DeviceDescriptor[0];
    ...
    case USB_DTYPE_CONFIGURATION: {
        ...
        if (ui8Index >= psDeviceDesc->bNumConfigurations) {
            ...
            //
            psUSBControl->ui32EP0DataRemain = USBDCDConfigDescGetSize(psConfig);
...
static int32_t USBDStringIndexFromRequest(uint16_t ui16Lang,
    ...
    // language code).
    //
    ui32NumLangs = (g_ppsDevInfo[0]->ppui8StringDescriptors[0][0] - 2) / 2;
    ...
    //
    pLang = (tString0Descriptor*)(g_ppsDevInfo[0]->ppui8StringDescriptors[0]);
...
static void USBDSetConfiguration(void* pvInstance, tUSBRequest* psUSBRequest) {
    ...
    if (psUSBRequest->wValue > psDevice->pui8DeviceDescriptor[17]) {
        ...
        if (psDevice->psCallbacks->pfnConfigChange) {
            psDevice->psCallbacks->pfnConfigChange(
                g_psDCDInst[0].pvCBData, psUSBControl->ui32Configuration);
        }
    }
}
...
static void USBDSetInterface(void* pvInstance, tUSBRequest* psUSBRequest) {
    ...
    for (ui32Loop = 0; ui32Loop < ui32NumInterfaces; ui32Loop++) {
        ...
        //
        psInterface = USBDCDConfigGetInterface(psConfig, ui32Loop, USB_DESC_ANY,
                                               &ui32Section);
        ...
        if (psInterface &&
            ...
            //
            bRetcode =
                USBDeviceConfigAlternate(psUSBControl, psConfig, ui8Interface,
                                         psInterface->bAlternateSetting);
            ...
            if (bRetcode && psDevice->psCallbacks->pfnInterfaceChange) {
                psDevice->psCallbacks->pfnInterfaceChange(
                    g_psDCDInst[0].pvCBData, psUSBRequest->wIndex,
                    psUSBRequest->wValue);
            }
    ...
    if (ui32NumBytes == EP0_MAX_PACKET_SIZE) {
        ...
        if ((g_ppsDevInfo[0]->psCallbacks->pfnDataSent) &&
            ...
            g_ppsDevInfo[0]->psCallbacks->pfnDataSent(
                g_psDCDInst[0].pvCBData, g_psDCDInst[0].ui32OUTDataSize);
...
static void USBDEP0StateTxConfig(uint32_t ui32Index) {
    ...
    //
    psConfig =
        g_ppsDevInfo[0]->ppsConfigDescriptors[g_psDCDInst[0].ui8ConfigIndex];
    ...
    if ((g_psDCDInst[0].ui16SectionOffset == 0) &&
        ...
        //
        ui32ToSend = (ui32NumBytes < sizeof(tConfigDescriptor))
                         ? ui32NumBytes
    ...
    while (ui32ToSend) {
        ...
        //
        ui32SecBytes =
            (uint32_t)(psSection->ui16Size - g_psDCDInst[0].ui16SectionOffset);
        ...
        //
        pui8Data =
            (uint8_t*)psSection->pui8Data + g_psDCDInst[0].ui16SectionOffset;
    ...
    if (g_psDCDInst[0].ui32EP0DataRemain != 0) {
        pui8Data =
            (uint8_t*)psConfig->psSections[g_psDCDInst[0].ui8ConfigSection]
                ->pui8Data;
...
void USBDeviceIntHandlerInternal(uint32_t ui32Index, uint32_t ui32Status) {
    ...
    if ((g_psDCDInst[0].ui32LPMState == USBLIB_LPM_STATE_SLEEP) &&
        ...
        if (g_ppsDevInfo[0]->psCallbacks->pfnDeviceHandler) {
            g_ppsDevInfo[0]->psCallbacks->pfnDeviceHandler(
                pvInstance, USB_EVENT_LPM_RESUME, (void*)0);
        }
    ...
    //
        ...
        if (g_ppsDevInfo[0]->psCallbacks->pfnDeviceHandler) {
            g_ppsDevInfo[0]->psCallbacks->pfnDeviceHandler(
                pvInstance, USB_EVENT_LPM_SLEEP, (void*)0);
        }
        ...
        if (g_ppsDevInfo[0]->psCallbacks->pfnDeviceHandler) {
            g_ppsDevInfo[0]->psCallbacks->pfnDeviceHandler(
                pvInstance, USB_EVENT_LPM_ERROR, (void*)0);
        }
    }
    ...
    if ((g_ppsDevInfo[0]->psCallbacks->pfnEndpointHandler) &&
        ...
        g_ppsDevInfo[0]->psCallbacks->pfnEndpointHandler(pvInstance,
                                                         ui32Status);
    }
}
