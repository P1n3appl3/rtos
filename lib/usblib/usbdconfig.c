...
//
//*****************************************************************************
static uint32_t GetEndpointFIFOSize(uint32_t ui32MaxPktSize,
...
//
//*****************************************************************************
static void GetEPDescriptorType(tEndpointDescriptor* psEndpoint,
                                uint32_t* pui32EPIndex,
                                uint32_t* pui32MaxPktSize,
                                uint32_t* pui32Flags) {
    //
...
bool USBDeviceConfig(tDCDInstance* psDevInst, const tConfigHeader* psConfig) {
    ...
    //
    ui32NumEndpoints = USBDCDConfigDescGetNum(psConfig, USB_DTYPE_ENDPOINT);
    ...
    for (ui32Loop = 0; ui32Loop < ui32NumEndpoints; ui32Loop++) {
        ...
        psEndpoint = (tEndpointDescriptor*)USBDCDConfigDescGet(
            psConfig, USB_DTYPE_ENDPOINT, ui32Loop, &ui32Section);
        ...
        //
        ui32EpIndex =
            (uint32_t)psEndpoint->bEndpointAddress & USB_EP_DESC_NUM_M;
        ...
        if (psInterface && (psInterface->bAlternateSetting == 0)) {
            ...
            for (ui32Count = 0; ui32Count < ui32NumEndpoints; ui32Count++) {
                ...
                psEndpoint = USBDCDConfigGetInterfaceEndpoint(
                    psConfig, psInterface->bInterfaceNumber,
                    psInterface->bAlternateSetting, ui32Count);
    ...
    for (ui32Loop = 1; ui32Loop < NUM_USB_EP; ui32Loop++) {
        ...
        if (psEPInfo[ui32Loop - 1].pui32Size[EP_INFO_IN]) {
            ...
            ui32MaxPkt = GetEndpointFIFOSize(
                psEPInfo[ui32Loop - 1].pui32Size[EP_INFO_IN], &ui32BytesUsed);
        ...
        if (psEPInfo[ui32Loop - 1].pui32Size[EP_INFO_OUT]) {
            ...
            //
            ui32MaxPkt = GetEndpointFIFOSize(
                psEPInfo[ui32Loop - 1].pui32Size[EP_INFO_OUT], &ui32BytesUsed);
...
bool USBDeviceConfigAlternate(tDCDInstance* psDevInst,
    ...
    for (ui32Loop = 0; ui32Loop < ui32NumInterfaces; ui32Loop++) {
        ...
        // Is this the default interface (bAlternateSetting set to 0)?
        //
        if (psInterface && (psInterface->bInterfaceNumber == ui8InterfaceNum) &&
