...
#include "tivaware/rom_map.h"
#include "tivaware/rtos_bindings.h"
#include "tivaware/sysctl.h"
#include "tivaware/usb.h"
...
#define OUT_PIPE_HANDLE(ui32Idx)                                               \
    (g_sUSBHCD.psUSBOUTPipes[ui32Idx].ui32Type | ui32Idx)
#define IN_PIPE_HANDLE(ui32Idx)                                                \
    (g_sUSBHCD.psUSBINPipes[ui32Idx].ui32Type | ui32Idx)
    ...
    if (g_sUSBHCD.psUSBDevice[0].psConfigDescriptor == 0) {
        ...
        for (ui32Idx = 1; ui32Idx < MAX_USB_DEVICES; ui32Idx++) {
            g_sUSBHCD.psUSBDevice[ui32Idx].psConfigDescriptor =
                (tConfigDescriptor*)(pui8Pool + (ui32Idx * ui32BlockSize));
...
void ConfigDescFree(tUSBHostDevice* psDevice) {
    ...
    if (&g_sUSBHCD.psUSBDevice[0] == psDevice) {
        for (ui32Idx = 0; ui32Idx < MAX_USB_DEVICES; ui32Idx++) {
            g_sUSBHCD.psUSBDevice[ui32Idx].ui32Flags &= ~USBHDEV_FLAG_ALLOCATED;
...
//
//*****************************************************************************
static uint8_t HCDInstanceToDevIndex(uint32_t ui32Instance) {
    uint32_t ui32DevIndex;
...
//
//=============================================================================
static uint32_t GetEventFlag(uint32_t ui32Event) {
...
//
//=============================================================================
int32_t USBHCDEventEnable(uint32_t ui32Index, void* pvEventDriver,
                          uint32_t ui32Event) {
    int32_t i32Ret;
    uint32_t ui32EventFlag;
...
//
//=============================================================================
int32_t USBHCDEventDisable(uint32_t ui32Index, void* pvEventDriver,
                           uint32_t ui32Event) {
    int32_t i32Ret;
    uint32_t ui32EventFlag;
...
//
//*****************************************************************************
static void SendUnknownConnect(uint32_t ui32Index, uint32_t ui32Class) {
    tEventInfo sEvent;
...
//
//*****************************************************************************
static void FIFOFree(tUSBHCDPipe* psUSBPipe) {
    uint32_t ui32Mask;
...
//
//*****************************************************************************
static uint32_t FIFOAlloc(tUSBHCDPipe* psUSBPipe, uint32_t ui32Size) {
...
//
//*****************************************************************************
uint32_t USBHCDPipeAllocSize(uint32_t ui32Index, uint32_t ui32EndpointType,
        ...
        if (ui32EndpointType & EP_PIPE_TYPE_OUT) {
            ...
            if (g_sUSBHCD.psUSBOUTPipes[i32Idx].psDevice == 0) {
                ...
                if (ui32EndpointType & EP_PIPE_USE_UDMA) {
                    ...
                    g_sUSBHCD.psUSBOUTPipes[i32Idx].ui8DMAChannel =
                        USBLibDMAChannelAllocate(
                            g_sUSBHCD.psDMAInstance, IndexToUSBEP(i32Idx + 1),
                            ui32Size, USB_DMA_EP_TX | USB_DMA_EP_HOST);
                ...
                MAP_USBHostEndpointStatusClear(
                    USB0_BASE, IndexToUSBEP(i32Idx + 1), USB_HOST_OUT_STATUS);
                ...
                //
                if (FIFOAlloc(&g_sUSBHCD.psUSBOUTPipes[i32Idx], ui32Size) !=
                    0) {
                    //
                    // Configure the FIFO.
                    //
                    MAP_USBFIFOConfigSet(
                        USB0_BASE, IndexToUSBEP(i32Idx + 1),
            ...
            if (g_sUSBHCD.psUSBINPipes[i32Idx].psDevice == 0) {
                ...
                if (ui32EndpointType & EP_PIPE_USE_UDMA) {
                    ...
                    g_sUSBHCD.psUSBINPipes[i32Idx].ui8DMAChannel =
                        USBLibDMAChannelAllocate(
                            g_sUSBHCD.psDMAInstance, IndexToUSBEP(i32Idx + 1),
                            ui32Size, USB_DMA_EP_RX | USB_DMA_EP_HOST);
                ...
                //
                MAP_USBHostEndpointStatusClear(
                    USB0_BASE, IndexToUSBEP(i32Idx + 1), USB_HOST_IN_STATUS);
                ...
                USBHostHubAddrSet(USB0_BASE, IndexToUSBEP(i32Idx + 1),
                                  ui32HubAddr,
                                  (USB_EP_HOST_IN | psDevice->ui32Speed));
...
//
//*****************************************************************************
uint32_t USBHCDPipeAlloc(uint32_t ui32Index, uint32_t ui32EndpointType,
                         tUSBHostDevice* psDevice,
                         tHCDPipeCallback pfnCallback) {
    //
...
//
//*****************************************************************************
uint32_t USBHCDPipeConfig(uint32_t ui32Pipe, uint32_t ui32MaxPayload,
    ...
    if (ui32Pipe & EP_PIPE_TYPE_OUT) {
        ...
        if (g_sUSBHCD.psUSBINPipes[ui32Index].ui32Type & EP_PIPE_TYPE_BULK) {
            ui32Flags = USB_EP_MODE_BULK;
        } else if (g_sUSBHCD.psUSBINPipes[ui32Index].ui32Type &
                   EP_PIPE_TYPE_INTR) {
            ui32Flags = USB_EP_MODE_INT;
        } else if (g_sUSBHCD.psUSBINPipes[ui32Index].ui32Type &
                   EP_PIPE_TYPE_ISOC) {
    ...
    USBHostEndpointConfig(
        USB0_BASE, IndexToUSBEP((ui32Pipe & EP_PIPE_IDX_M) + 1), ui32MaxPayload,
        ui32Interval, ui32TargetEndpoint, ui32Flags);
...
//
//*****************************************************************************
uint32_t USBHCDPipeWrite(uint32_t ui32Pipe, uint8_t* pui8Data,
                         uint32_t ui32Size) {
    ...
    while (ui32RemainingBytes != 0) {
        ...
        if (ui32Pipe & EP_PIPE_USE_UDMA) {
            ...
            if (USBLibDMATransfer(
                ...
                if (ui32RemainingBytes < 64) {
                    ...
                    g_sUSBHCD.psUSBOUTPipes[ui32PipeIdx].iState = ePipeWriteDMA;
                } else {
                    g_sUSBHCD.psUSBOUTPipes[ui32PipeIdx].iState =
                        ePipeWriteDMASend;
                }
        ...
        if (bUseDMA == false) {
            ...
            //
            MAP_USBEndpointDMADisable(USB0_BASE, ui32Endpoint, USB_EP_HOST_OUT);
        ...
        while (1) {
            ...
            if (g_sUSBHCD.ui32IntEvents &
                (INT_EVENT_DISCONNECT | INT_EVENT_VBUS_ERR |
...
//
//*****************************************************************************
uint32_t USBHCDPipeSchedule(uint32_t ui32Pipe, uint8_t* pui8Data,
                            uint32_t ui32Size) {
    ...
    if (ui32Pipe & EP_PIPE_TYPE_OUT) {
        ...
        if (ui32Pipe & EP_PIPE_USE_UDMA) {
            ...
            USBLibDMATransfer(
                g_sUSBHCD.psDMAInstance,
                g_sUSBHCD.psUSBOUTPipes[ui32PipeIdx].ui8DMAChannel, pui8Data,
                ui32Size);
        } else {
            //
            // Start a write request.
            ...
            // Put the data in the buffer.
            //
            MAP_USBEndpointDataPut(USB0_BASE, ui32Endpoint, pui8Data, ui32Size);
...
//
//*****************************************************************************
uint32_t USBHCDPipeReadNonBlocking(uint32_t ui32Pipe, uint8_t* pui8Data,
                                   uint32_t ui32Size) {
...
//
//*****************************************************************************
uint32_t USBHCDPipeRead(uint32_t ui32Pipe, uint8_t* pui8Data,
                        uint32_t ui32Size) {
    ...
    while (ui32RemainingBytes != 0) {
        ...
        if (ui32Pipe & EP_PIPE_USE_UDMA) {
            ...
            if (USBLibDMATransfer(
                    g_sUSBHCD.psDMAInstance,
                    g_sUSBHCD.psUSBINPipes[ui32PipeIdx].ui8DMAChannel, pui8Data,
                    ui32Size) != 0) {
...
void USBHCDPipeFree(uint32_t ui32Pipe) {
    ...
    if (ui32Pipe & EP_PIPE_TYPE_OUT) {
        ...
        //
        USBHostAddrSet(USB0_BASE, IndexToUSBEP(ui32Index + 1), 0,
                       USB_EP_HOST_OUT);
        ...
        // speed back to LOW.
        //
        USBHostHubAddrSet(USB0_BASE, IndexToUSBEP(ui32Index + 1), 0,
                          (USB_EP_HOST_OUT | USB_EP_SPEED_LOW));
        ...
        if (g_sUSBHCD.psUSBINPipes[ui32Index].ui8DMAChannel !=
            ...
            // Clear out the current channel in use by this pipe.
            //
            g_sUSBHCD.psUSBINPipes[ui32Index].ui8DMAChannel = USBHCD_DMA_UNUSED;
        }
        ...
        //
        USBHostHubAddrSet(USB0_BASE, IndexToUSBEP(ui32Index + 1), 0,
                          (USB_EP_HOST_IN | USB_EP_SPEED_LOW));
...
//
//*****************************************************************************
static void USBHCDInitInternal(uint32_t ui32Index, void* pvPool,
                               uint32_t ui32PoolSize) {
    int32_t i32Idx;
    ...
    // event notifications.
    //
    g_sUSBHCD.ui32EventEnables =
        USBHCD_EVFLAG_CONNECT | USBHCD_EVFLAG_UNKCNCT | USBHCD_EVFLAG_DISCNCT |
    ...
    if ((g_iUSBMode == eUSBModeHost) || (g_iUSBMode == eUSBModeForceHost)) {
        ...
        MAP_USBIntEnableControl(
            USB0_BASE, USB_INTCTRL_RESET | USB_INTCTRL_DISCONNECT |
                           USB_INTCTRL_SOF | USB_INTCTRL_SESSION |
                           USB_INTCTRL_BABBLE | USB_INTCTRL_CONNECT |
                           USB_INTCTRL_RESUME | USB_INTCTRL_SUSPEND |
        ...
        //
        USBHCDPowerConfigSet(ui32Index,
                             (g_ui32PowerConfig & ~USB_HOST_PWREN_AUTO));
...
uint32_t USBHCDPowerConfigSet(uint32_t ui32Index, uint32_t ui32Config) {
    ...
    //
    ui32Config =
        g_ui32PowerConfig & ~(USBHCD_VBUS_MANUAL | USBHCD_FAULT_VBUS_DIS);
...
void USBHCDInit(uint32_t ui32Index, void* pvPool, uint32_t ui32PoolSize) {
    ...
    //
    for (i32Driver = 0; i32Driver < g_sUSBHCD.ui32NumClassDrivers;
         i32Driver++) {
...
//
//*****************************************************************************
static uint32_t USBHCDGetConfigDescriptor(uint32_t ui32Index,
                                          tUSBHostDevice* psDevice) {
    ...
    if (!psDevice->bConfigRead) {
        ...
        //
        ui32Bytes = USBHCDControlTransfer(
            0, &sSetupPacket, psDevice, (uint8_t*)&sConfigDescriptor,
    ...
    if (ui32Bytes == sizeof(tConfigDescriptor)) {
        ...
        //
        ui32Bytes = USBHCDControlTransfer(
            0, &sSetupPacket, psDevice, (uint8_t*)psDevice->psConfigDescriptor,
            sSetupPacket.wLength, psDevice->sDeviceDescriptor.bMaxPacketSize0);
...
//
//*****************************************************************************
static uint32_t USBHCDGetDeviceDescriptor(uint32_t ui32Index,
                                          tUSBHostDevice* psDevice) {
    tUSBRequest sSetupPacket;
    uint32_t ui32Bytes;
    ...
    if (ui32Bytes < sizeof(tDeviceDescriptor)) {
        ...
        ui32Bytes = USBHCDControlTransfer(
            ui32Index, &sSetupPacket, psDevice,
            (uint8_t*)&(psDevice->sDeviceDescriptor), sizeof(tDeviceDescriptor),
            psDevice->sDeviceDescriptor.bMaxPacketSize0);
    }
...
uint32_t USBHCDStringDescriptorGet(tUSBHostDevice* psDevice,
    ...
    //
    sSetupPacket.wValue = (USB_DTYPE_STRING << 8) | (uint16_t)ui32StringIndex;
...
void USBHCDSetAddress(uint32_t ui32DevIndex, uint32_t ui32DevAddress) {
    ...
    USBHCDControlTransfer(0, &sSetupPacket,
                          &g_sUSBHCD.psUSBDevice[ui32DevIndex], 0, 0,
                          MAX_PACKET_SIZE_EP0);
...
//
//*****************************************************************************
void USBHCDClearFeature(uint32_t ui32DevAddress, uint32_t ui32Pipe,
                        uint32_t ui32Feature) {
    tUSBRequest sSetupPacket;
    uint32_t ui32Index;
    ...
    if (ui32Pipe & EP_PIPE_TYPE_IN) {
        sSetupPacket.wIndex =
            g_sUSBHCD.psUSBINPipes[ui32Index].ui8EPNumber | 0x80;
    } else {
    ...
    //
    if (ui32Pipe & EP_PIPE_TYPE_IN) {
        MAP_USBEndpointDataToggleClear(USB0_BASE, IndexToUSBEP(ui32Index + 1),
                                       USB_EP_HOST_IN);
    } else {
        MAP_USBEndpointDataToggleClear(USB0_BASE, IndexToUSBEP(ui32Index + 1),
                                       USB_EP_HOST_OUT);
...
//
//*****************************************************************************
void USBHCDSetConfig(uint32_t ui32Index, uint32_t ui32Device,
...
//
//*****************************************************************************
void USBHCDSetInterface(uint32_t ui32Index, uint32_t ui32Device,
    ...
    for (i32Idx = 0; i32Idx < g_sUSBHCD.ui32NumEndpoints; i32Idx++) {
        ...
        if ((g_sUSBHCD.psUSBINPipes[i32Idx].ui32Interval != 0) &&
            ...
            if ((g_sUSBHCD.psUSBINPipes[i32Idx].iState == ePipeIdle) &&
                ...
                g_sUSBHCD.psUSBINPipes[i32Idx].pfnCallback(
                    IN_PIPE_HANDLE(i32Idx), USB_EVENT_SCHEDULER);
            }
        }
    }
}
...
void USBHostIntHandlerInternal(uint32_t ui32Index, uint32_t ui32Status) {
    ...
    if (ui32DMAIntStatus) {
        ...
        for (ui32Idx = 0; ui32Idx < MAX_NUM_PIPES; ui32Idx++) {
            ...
            if (g_sUSBHCD.psUSBOUTPipes[ui32Idx].iState == ePipeWriteDMASend) {
                //
                ...
                if (USBLibDMAChannelStatus(
                    ...
                    MAP_USBEndpointDataSend(
                        USB0_BASE, IndexToUSBEP(ui32Idx + 1), USB_TRANS_OUT);
                ...
                if (g_sUSBHCD.psUSBOUTPipes[ui32Idx].pfnCallback) {
                    ...
                    g_sUSBHCD.psUSBOUTPipes[ui32Idx].pfnCallback(
                        OUT_PIPE_HANDLE(ui32Idx), USB_EVENT_TX_COMPLETE);
                }
            }
        }
    }
    ...
    for (ui32Idx = 0; ui32Idx < MAX_NUM_PIPES; ui32Idx++) {
        ...
        if (ui32Status & 0x10000) {
            ...
            //
            ui32EPStatus =
                MAP_USBEndpointStatus(USB0_BASE, IndexToUSBEP(ui32Idx + 1));
            ...
            if (ui32EPStatus & USB_HOST_IN_STALL) {
                ...
                //
                MAP_USBHostEndpointStatusClear(
                    USB0_BASE, IndexToUSBEP(ui32Idx + 1), USB_HOST_IN_STALL);
                ...
                if (g_sUSBHCD.psUSBINPipes[ui32Idx].pfnCallback) {
                    g_sUSBHCD.psUSBINPipes[ui32Idx].pfnCallback(
                        IN_PIPE_HANDLE(ui32Idx), USB_EVENT_STALL);
                }
                ...
                //
                USBHostEndpointStatusClear(USB0_BASE, IndexToUSBEP(ui32Idx + 1),
                                           USB_HOST_IN_ERROR);
                ...
                if (g_sUSBHCD.psUSBINPipes[ui32Idx].pfnCallback) {
                    g_sUSBHCD.psUSBINPipes[ui32Idx].pfnCallback(
                        IN_PIPE_HANDLE(ui32Idx), USB_EVENT_ERROR);
                }
            }
            //
            ...
            //
                ...
                g_sUSBHCD.psUSBINPipes[ui32Idx].ui32DataRead =
                    USBEndpointDataAvail(USB0_BASE, IndexToUSBEP(ui32Idx + 1));
                ...
                if (g_sUSBHCD.psUSBINPipes[ui32Idx].ui32DataRead <=
                    ...
                    USBLibDMATransfer(
                        g_sUSBHCD.psDMAInstance,
                        g_sUSBHCD.psUSBINPipes[ui32Idx].ui8DMAChannel, pvAddr,
                        g_sUSBHCD.psUSBINPipes[ui32Idx].ui32DataRead);
                } else {
                    //
                ...
                if (g_sUSBHCD.psUSBINPipes[ui32Idx].pui8ReadPtr) {
                    ...
                    USBEndpointDataGet(
                        USB0_BASE, IndexToUSBEP(ui32Idx + 1),
        ...
        if (ui32Status & 1) {
            ...
            if (ui32EPStatus & USB_HOST_OUT_STALL) {
                ...
                //
                MAP_USBHostEndpointStatusClear(
                    USB0_BASE, IndexToUSBEP(ui32Idx + 1), USB_HOST_OUT_STALL);
                ...
                if (g_sUSBHCD.psUSBOUTPipes[ui32Idx].pfnCallback) {
                    ...
                    //
                    g_sUSBHCD.psUSBOUTPipes[ui32Idx].pfnCallback(
                        OUT_PIPE_HANDLE(ui32Idx), USB_EVENT_STALL);
                }
                ...
                //
                MAP_USBHostEndpointStatusClear(
                    USB0_BASE, IndexToUSBEP(ui32Idx + 1), USB_HOST_OUT_ERROR);
                ...
                if (g_sUSBHCD.psUSBOUTPipes[ui32Idx].pfnCallback) {
                    ...
                    //
                    g_sUSBHCD.psUSBOUTPipes[ui32Idx].pfnCallback(
                        OUT_PIPE_HANDLE(ui32Idx), USB_EVENT_ERROR);
                }
            } else if ((g_sUSBHCD.psUSBOUTPipes[ui32Idx].iState ==
                        ePipeWriting) ||
            ...
            // Clear the stall condition on this endpoint pipe.
            //
            MAP_USBHostEndpointStatusClear(USB0_BASE, IndexToUSBEP(ui32Idx + 1),
...
static int32_t USBHCDOpenDriver(uint32_t ui32Index, uint32_t ui32DeviceNum) {
    ...
    psInterface = USBDescGetInterface(
        ...
        g_sUSBHCD.psUSBDevice[ui32DeviceNum].ui32Interface, USB_DESC_ANY);
...
//
//*****************************************************************************
void InternalUSBHCDSendEvent(uint32_t ui32Index, tEventInfo* psEvent,
                             uint32_t ui32EvFlag) {
    //
...
//
//*****************************************************************************
static void USBHCDDeviceDisconnected(uint32_t ui32Index,
                                     uint32_t ui32DevIndex) {
    tEventInfo sEvent;
            ...
            if (g_sUSBHCD.ui32IntEvents & INT_EVENT_LPM) {
                ...
                for (i32Dev = 0; i32Dev < MAX_USB_DEVICES + 1; i32Dev++) {
                    ...
                    if (g_sUSBHCD.psUSBDevice[i32Dev].ui32Flags &
                        ...
                        // controller level.
                        //
                        g_sUSBHCD.ui32IntEvents &=
                            ~(INT_EVENT_LPM_PEND | INT_EVENT_LPM);
                    }
                }
            }
        }
...
static void ProcessUSBDeviceStateMachine(tUSBHDeviceState iOldState,
    ...
    case eHCDDevConnectedHub: {
        ...
        if (g_sUSBHCD.psUSBDevice[ui32DevIndex].sDeviceDescriptor.bLength ==
            ...
            if (USBHCDGetDeviceDescriptor(
                    0, &g_sUSBHCD.psUSBDevice[ui32DevIndex]) == 0) {
                //
    ...
    case eHCDDevAddressed: {
        ...
        //
            ...
            // configuration.
            //
            USBHCDSetConfig(0, (uint32_t)&g_sUSBHCD.psUSBDevice[ui32DevIndex],
                            1);
            ...
            //
            g_pi32USBHActiveDriver[ui32DevIndex] =
                USBHCDOpenDriver(0, ui32DevIndex);
...
//
//*****************************************************************************
uint32_t USBHCDControlTransfer(uint32_t ui32Index, tUSBRequest* psSetupPacket,
    ...
    USBHostEndpointConfig(
        USB0_BASE, USB_EP_0, 64, 0, 0,
        (USB_EP_MODE_CTRL | psDevice->ui32Speed | USB_EP_HOST_OUT));
    ...
    while (g_sUSBHEP0State.iState != eEP0StateIdle) {
        ...
        //
        if (g_sUSBHCD.ui32IntEvents &
            (INT_EVENT_VBUS_ERR | INT_EVENT_DISCONNECT)) {
            return (0xffffffff);
        }
    }
...
//
//*****************************************************************************
uint32_t USBHCDHubDeviceConnected(uint32_t ui32Index, uint8_t ui8Hub,
...
uint8_t USBHCDDevClass(uint32_t ui32Instance, uint32_t ui32Interface) {
    ...
    psInterface = USBDescGetInterface(
        ...
        g_sUSBHCD.psUSBDevice[ui32DevIndex].ui32Interface, ui32Interface);
...
//
//*****************************************************************************
bool USBHCDFeatureSet(uint32_t ui32Index, uint32_t ui32Feature,
                      void* pvFeature) {
    bool bRetCode;
