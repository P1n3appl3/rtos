...
//
//*****************************************************************************
const tUSBHostClassDriver g_sUSBHubClassDriver = {USB_CLASS_HUB, HubDriverOpen,
                                                  HubDriverClose, 0};
...
//
//*****************************************************************************
static void HubSetPortFeature(tHubInstance* psHubInstance, uint8_t ui8Port,
    ...
    sSetupPacket.bmRequestType =
        USB_RTYPE_DIR_OUT | USB_RTYPE_CLASS | USB_RTYPE_OTHER;
...
//
//*****************************************************************************
static void HubClearPortFeature(tHubInstance* psHubInstance, uint8_t ui8Port,
                                uint16_t ui16Feature) {
    tUSBRequest sSetupPacket;
    tUSBHostDevice* psDevice;
...
//
//*****************************************************************************
static bool HubGetPortStatus(tHubInstance* psHubInstance, uint8_t ui8Port,
                             uint16_t* pui16PortStatus,
                             uint16_t* pui16PortChange) {
    ...
    //
    sSetupPacket.bmRequestType =
        USB_RTYPE_DIR_IN | USB_RTYPE_CLASS | USB_RTYPE_OTHER;
    ...
    //
    ui32Read =
        USBHCDControlTransfer(0, &sSetupPacket, psDevice, (uint8_t*)&ui32Data,
                              4, psDevice->sDeviceDescriptor.bMaxPacketSize0);
    ...
    case USB_EVENT_RX_AVAILABLE: {
        ...
        if (g_sRootHub.pfnCallback) {
            g_sRootHub.pfnCallback((void*)g_sRootHub.ui32CBData,
                                   USB_EVENT_RX_AVAILABLE, ui32Pipe,
                                   &g_ui32HubChanges);
        }
...
static bool GetHubDescriptor(tUsbHubDescriptor* psDesc) {
    ...
    //
    sSetupPacket.bmRequestType =
        USB_RTYPE_DIR_IN | USB_RTYPE_CLASS | USB_RTYPE_DEVICE;
    ...
    ui32Read = USBHCDControlTransfer(
        0, &sSetupPacket, psDevice, (void*)psDesc, sizeof(tUsbHubDescriptor),
        psDevice->sDeviceDescriptor.bMaxPacketSize0);
...
static void* HubDriverOpen(tUSBHostDevice* psDevice) {
    ...
    psEndpointDescriptor = USBDescGetInterfaceEndpoint(
        psInterface, 0, psDevice->ui32ConfigDescriptorSize);
    ...
    if ((psEndpointDescriptor->bmAttributes & USB_EP_ATTR_TYPE_M) ==
        ...
        if (psEndpointDescriptor->bEndpointAddress & USB_EP_DESC_IN) {
            ...
            //
            g_sRootHub.ui32IntInPipe = USBHCDPipeAlloc(
                0, USBHCD_PIPE_INTR_IN, psDevice, HubIntINCallback);
            USBHCDPipeConfig(
                g_sRootHub.ui32IntInPipe, psEndpointDescriptor->wMaxPacketSize,
    ...
    if (bRetcode) {
        ...
        g_sRootHub.ui8NumPortsInUse = (sHubDesc.bNbrPorts > MAX_USB_DEVICES)
                                          ? MAX_USB_DEVICES
                                          : sHubDesc.bNbrPorts;
    ...
    for (ui32Loop = 0; ui32Loop < MAX_USB_DEVICES; ui32Loop++) {
        ...
        if ((g_sRootHub.psPorts[ui32Loop].iState == ePortActive) ||
            ...
            USBHCDHubDeviceDisconnected(
                0, g_sRootHub.psPorts[ui32Loop].ui32DevHandle);
        }
...
static void HubDriverReset(uint8_t ui8Port, bool bResetActive) {
    ...
    if (!bResetActive) {
        ...
        if (g_sRootHub.psPorts[ui8Port].iState == ePortActive) {
            USBHCDHubDeviceDisconnected(
                0, g_sRootHub.psPorts[ui8Port].ui32DevHandle);
        }
        ...
        if (g_ui32ChangeFlags & (1 << ui8Port)) {
            ...
            if (ui16Changed & HUB_PORT_CHANGE_RESET) {
                ...
                DEBUG_OUTPUT("Reset %s for port %d\n",
                             ((ui16Status & HUB_PORT_STATUS_RESET)
                                  ? "asserted"
                                  : "deasserted"),
                             ui8Port);
