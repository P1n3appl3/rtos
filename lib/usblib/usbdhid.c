...
#define USB_RX_ERROR_FLAGS                                                     \
    (USBERR_DEV_RX_DATA_ERROR | USBERR_DEV_RX_OVERRUN | USBERR_DEV_RX_FIFO_FULL)
...
//
//*****************************************************************************
static void SetDeferredOpFlag(volatile uint16_t* pui16DeferredOp,
                              uint16_t ui16Bit, bool bSet) {
    //
...
//
//*****************************************************************************
static void ClearReportTimer(const tUSBDHIDDevice* psHIDDevice,
                             uint8_t ui8ReportID) {
    uint32_t ui32Loop;
    ...
    if (psHIDDevice->ui8NumInputReports > 1) {
        ...
        for (ui32Loop = 0; ui32Loop < psHIDDevice->ui8NumInputReports;
             ui32Loop++) {
            if (psHIDDevice->psReportIdle[ui32Loop].ui8ReportID ==
                ui8ReportID) {
                break;
            }
        }
...
//
//*****************************************************************************
static void ProcessIdleTimers(const tUSBDHIDDevice* psHIDDevice,
                              uint32_t ui32ElapsedmS) {
            ...
            if (psHIDDevice->psReportIdle[ui32Loop].ui16TimeTillNextmS <=
                ...
                if ((psInst->iHIDTxState == eHIDStateIdle) &&
                    ...
                    ui32SizeReport = psHIDDevice->pfnRxCallback(
                        psHIDDevice->pvRxCBData, USBD_HID_EVENT_IDLE_TIMEOUT,
...
static void SetIdleTimeout(const tUSBDHIDDevice* psHIDDevice,
                           uint8_t ui8ReportID, uint8_t ui8Timeout4mS) {
    uint32_t ui32Loop;
    bool bReportNeeded;
...
//
//*****************************************************************************
static uint32_t GetIdleTimeout(const tUSBDHIDDevice* psHIDDevice,
                               uint8_t ui8ReportID) {
    uint32_t ui32Loop;
    tHIDReportIdle* psIdle;
...
//
//*****************************************************************************
static uint32_t FindHIDDescriptor(const tUSBDHIDDevice* psHIDDevice,
                                  uint8_t ui8Type, uint32_t ui32Index,
                                  uint32_t* pui32Len) {
    bool bFoundType;
...
//
//*****************************************************************************
static int32_t ScheduleReportTransmission(tHIDInstance* psHIDInst) {
    ...
    //
    ui32NumBytes =
        (uint32_t)(psHIDInst->ui16InReportSize - psHIDInst->ui16InReportIndex);
    ...
    //
    i32Retcode =
        MAP_USBEndpointDataPut(psHIDInst->ui32USBBase, psHIDInst->ui8INEndpoint,
    ...
    if (i32Retcode != -1) {
        ...
        //
        i32Retcode = MAP_USBEndpointDataSend(
            psHIDInst->ui32USBBase, psHIDInst->ui8INEndpoint, USB_TRANS_IN);
    }
...
//
//*****************************************************************************
static bool ProcessDataFromHost(tUSBDHIDDevice* psHIDDevice,
                                uint32_t ui32Status) {
    ...
    if (ui32EPStatus & USB_DEV_RX_PKT_RDY) {
        ...
        psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData,
                                   USB_EVENT_RX_AVAILABLE, ui32Size, (void*)0);
    } else {
        //
        ...
        if (ui32EPStatus & USB_RX_ERROR_FLAGS) {
            ...
            //
            psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_ERROR,
...
//
//*****************************************************************************
static bool ProcessDataToHost(tUSBDHIDDevice* psHIDDevice,
                              uint32_t ui32Status) {
    tHIDInstance* psInst;
    ...
    //
    ui32EPStatus =
        MAP_USBEndpointStatus(psInst->ui32USBBase, psInst->ui8INEndpoint);
...
static void HandleConfigChange(void* pvHIDInstance, uint32_t ui32Info) {
    ...
    if (!psInst->bConnected) {
        ...
        //
        psHIDDevice->pfnRxCallback(psHIDDevice->pvRxCBData, USB_EVENT_CONNECTED,
                                   0, (void*)0);
    }
...
//
//*****************************************************************************
static void HandleGetDescriptor(void* pvHIDInstance,
    ...
    case USB_HID_DTYPE_PHYSICAL: {
        ...
        //
        ui32Size = 0;
        ui32Desc = FindHIDDescriptor(psHIDDevice, psUSBRequest->wValue >> 8,
                                     psUSBRequest->wValue & 0xFF, &ui32Size);
        ...
        //
        USBDCDSendDataEP0(
            0, (uint8_t*)psHIDDevice->ppui8ClassDescriptors[ui32Desc],
            ui32Size);
    ...
    case USB_HID_DTYPE_HID: {
        ...
        // Send the data via endpoint 0.
        //
        USBDCDSendDataEP0(0, (uint8_t*)psHIDDevice->psHIDDescriptor, ui32Size);
        break;
    }
...
static void HandleRequest(void* pvHIDInstance, tUSBRequest* psUSBRequest) {
    ...
    case USBREQ_SET_REPORT: {
        ...
        psInst->pui8OutReportData = (uint8_t*)psHIDDevice->pfnRxCallback(
            psHIDDevice->pvRxCBData, USBD_HID_EVENT_GET_REPORT_BUFFER,
            psUSBRequest->wValue, (void*)(uint32_t)(psUSBRequest->wLength));
    ...
    case USBREQ_GET_REPORT: {
        ...
        //
        ui32Size = psHIDDevice->pfnRxCallback(
            psHIDDevice->pvRxCBData, USBD_HID_EVENT_GET_REPORT,
...
static void HandleEP0DataReceived(void* pvHIDInstance, uint32_t ui32DataSize) {
    ...
    // the client.
    //
    psHIDDevice->pfnRxCallback(
        psHIDDevice->pvRxCBData, USBD_HID_EVENT_SET_REPORT,
        psInst->ui16OutReportSize, psInst->pui8OutReportData);
}
...
void* USBDHIDInit(uint32_t ui32Index, tUSBDHIDDevice* psHIDDevice) {
    ...
    //
    pi16DevDesc = (tDeviceDescriptor*)
                      psHIDDevice->sPrivateData.sDevInfo.pui8DeviceDescriptor;
...
//
//*****************************************************************************
void* USBDHIDCompositeInit(uint32_t ui32Index, tUSBDHIDDevice* psHIDDevice,
    ...
    psEndpoint = USBDCDConfigGetInterfaceEndpoint(
        psHIDDevice->ppsConfigDescriptor[0], psInst->ui8Interface, 0, 0);
    ...
    //
    psEndpoint = USBDCDConfigGetInterfaceEndpoint(
        psHIDDevice->ppsConfigDescriptor[0], psInst->ui8Interface, 0, 1);
    if (psEndpoint) {
        if (psEndpoint->bEndpointAddress & 0x80) {
            psInst->ui8INEndpoint = IndexToUSBEP(psEndpoint->bEndpointAddress);
        } else {
...
//
//*****************************************************************************
uint32_t USBDHIDReportWrite(void* pvHIDInstance, uint8_t* pi8Data,
...
//
//*****************************************************************************
uint32_t USBDHIDPacketRead(void* pvHIDInstance, uint8_t* pi8Data,
                           uint32_t ui32Length, bool bLast) {
    ...
    //
    ui32EPStatus =
        MAP_USBEndpointStatus(psInst->ui32USBBase, psInst->ui8OUTEndpoint);
    if (ui32EPStatus & USB_DEV_RX_PKT_RDY) {
        //
        ...
        i32Retcode = MAP_USBEndpointDataGet(
            psInst->ui32USBBase, psInst->ui8OUTEndpoint, pi8Data, &ui32Count);
        ...
        if (ui32Count == ui32Pkt) {
            ...
            MAP_USBDevEndpointStatusClear(psInst->ui32USBBase,
                                          psInst->ui8OUTEndpoint, ui32EPStatus);
