...
//
//*****************************************************************************
static tDescriptorHeader* NextConfigDescGet(const tConfigHeader* psConfig,
                                            uint32_t* pui32Sec,
...
tInterfaceDescriptor* USBDCDConfigGetInterface(const tConfigHeader* psConfig,
    ...
    if (ui32Alt == USB_DESC_ANY) {
        ...
        // configuration descriptor.
        //
        return ((tInterfaceDescriptor*)USBDCDConfigDescGet(
            psConfig, USB_DTYPE_INTERFACE, ui32Index, pui32Section));
    } else {
        //
    ...
    if (psInterface) {
        ...
        if (ui32Index >= psInterface->bNumEndpoints) {
            ...
            while (psEndpoint) {
                ...
                //
                psEndpoint =
                    NextConfigDescGet(psConfig, &ui32Section, psEndpoint);
            }
        }
    }
